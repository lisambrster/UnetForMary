...
from dataset_Rhabdo import Rhabdo
import argparse
import cv2
...
class InstanceHeat(object):
    def __init__(self):
        ...
        self.dataset = {'kaggle': Kaggle, 'plant': Plant, 'neural': Neural, 'Rhabdo': Rhabdo}
    ...
    def load_weights(self, resume, dataset):
        ...
        self.model.load_state_dict( torch.load( os.path.join('weights_' + dataset, resume), map_location=torch.device('cpu') ) )
    ...
    def test(self, args):
        ...
        imgFile = '/Users/lisabrown/Mary/InstanceSegmentation/Rhabdo/E13_0001/images/E13_0001.png'
        rgb = cv2.imread(imgFile)
        print(rgb.shape,type(rgb))


        imgFile = '/Users/lisabrown/Mary/MaryData/ERMS/tifs/Whitney 9-15-Scene-111-P1-G02.tif'
        from skimage import io
        seq = io.imread(imgFile, plugin='pil')
        print(seq.shape)
        h = seq.shape[1]
        w = seq.shape[2]
        nframes = seq.shape[0]
        ...
        for iframe in range(30,nframes):
            img = seq[iframe,:,:]
            rgb = cv2.cvtColor(img, cv2.COLOR_GRAY2RGB)
            rgb = cv2.convertScaleAbs(rgb, alpha=(255.0/65535.0))

            # need to make it rgb ?
            predictions = self.test_inference(args, rgb)
            if predictions is None:
                print('no predictions')
            else:
                mask_patches, mask_dets = predictions
                # dets look like x1,y1,x2,y2, prob ? and mask patches probably binary
                #print(len(mask_dets),mask_dets.shape,mask_patches.shape,mask_dets[2],mask_patches[1,1,1])
                print(save_path)
                img_id = '%04d' % iframe # get from frame of tiff..
                out_img = rgb.copy()
                colors = random_colors(mask_dets.shape[0])
                imask = 0
                for mask, det, color in zip(mask_patches, mask_dets, colors):
                    color = np.random.rand(3)
                    print(mask_dets[imask])
                    imask = imask + 1
                    apply_mask(image=out_img, mask=mask, color=color, alpha=0.8)
                print(save_path)
                cv2.imwrite(os.path.join(save_path, img_id + ".png"), np.uint8(out_img))
