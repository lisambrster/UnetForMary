...
class InstanceHeat(object):
    def __init__(self):
        ...
        if torch.cuda.is_available():
            print('torch cuda available 2')
        else:
            print('No torch cuda -- using cpu')
        self.device = torch.device("cuda:2" if torch.cuda.is_available() else "cpu")
        self.dataset = {'kaggle': Kaggle, 'plant': Plant, 'neural': Neural, 'Rhabdo': Rhabdo}

    def data_parallel(self):
        print('using data parallel')
        self.model = torch.nn.DataParallel(self.model) # try without cuda
        #self.model = torch.nn.DataParallel(self.model, device_ids=[0, 1, 2, 3]).cuda()  # add device_ids LB
    ...
    def train(self, args):
        ...
        data_trans = {'train': transforms.Compose([transforms.ConvertImgFloat(),
                                                   ...
                                                   transforms.RandomLightingNoise(),
    ...
    def training(self, train_loader, loss_dec, loss_seg, optimizer, epoch, dsets):
        #print ('training: entering')
        self.model.train()
        running_loss = 0.0

        for data in train_loader:
            #print ('training: top of train_loader loop')
            img, gt_c0, gt_c1, gt_c2, gt_c3, instance_masks, bboxes_c0  = data
            #print ('training: after splitting data')
            ...
            #print('training: before model / backward / optimizer block')
            ...
            #print ('training: bottom of train_loader loop')
        #print ('training: exiting, return 0')
        #return 0
        epoch_loss = running_loss / len(dsets)
        print('{} Loss: {:.6}'.format(epoch, epoch_loss))
        #print ('training: exiting, return epoch_loss')
        return epoch_loss
